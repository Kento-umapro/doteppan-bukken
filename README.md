...
### 掲載の精度基準（2026-08-20 精度優先に変更）
掲載件数が増えすぎたため、掲載は偏差値と予測倍率で厳選する:
- **標準物件（prefs）**: 出店偏差値 **50以上** かつ 予測倍率 **10倍以上**（採算が明確に見込める「狙い目」）
- **上級物件（premium）**: 出店偏差値 **50以上** のみ。高坪単価で家賃が重く倍率が10倍に届きにくい枠のため、
  倍率10倍ゲートは**免除**（立地の集客力で採算を取る1等立地物件）。

**巡回・収集は従来どおり広く行う**（重点7府県＝大阪・京都・滋賀・愛知・群馬・埼玉・千葉は
17坪〜で広めに収集）。基準を外れた候補は削除せず、5km圏内は `hungree/extra.json`、
